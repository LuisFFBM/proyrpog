...
int main(int argc, char **argv)
{
    ...
    if (all || test == 15) test1_link_set_open();
    if (all || test == 16) test2_link_set_open();
    if (all || test == 17) test3_link_set_open();
    if (all || test == 18) test4_link_set_open();
    if (all || test == 19) test1_link_get_open();
    if (all || test == 20) test2_link_get_open();
    if (all || test == 21) test3_link_get_open();
...
void test1_link_set_open()
{
    Link *l= NULL;
    l = link_create(5);
    PRINT_TEST_RESULT(link_set_open(l,FALSE) == OK && link_get_open(l) == FALSE);
    link_destroy(l);
...
void test2_link_set_open()
{
    Link *l= NULL;
    l = link_create(5);
    PRINT_TEST_RESULT(link_set_open(l,(-1)) == ERROR);
    link_destroy(l);
...
void test3_link_set_open()
{
    Link *l= NULL;
    l = link_create(5);
    PRINT_TEST_RESULT(link_set_open(l,2) == ERROR);
    link_destroy(l);

}

void test4_link_set_open()
{
    Link *l= NULL;
