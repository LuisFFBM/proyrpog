...
BOOL slot_isFull(Slot *slot) {
    if(slot == NULL){
        return FALSE;
    }

    if(slot->n_slots == MAX_SLOTS){
        return TRUE;
    }
...
STATUS slot_push(Slot *slot, char*filename, int pos){
    int i=0;
    FILE *file = NULL;
    ...
    if(slot_isEmpty == TRUE)
    {
        slot->slot[0] = filename;
        slot->n_slots ++;
    }
    else if(slot_isFull == TRUE)
    {
        for(i=0; i< MAX_SLOTS; i++){
            if(strcmp(slot->slot[i], filename)){
...
STATUS game_load_from_file(Game **game, char *filename) {
    ...
    /*
    game_set_slot(game_new, game_get_slots(*game));
    
    game_set_last_command(game_new,game_get_last_command(*game));
    */
    game_destroy(*game);
