...
struct _Player
{
    ...
    Inventory *inv;  /*!< The inventory (riñonera) with objects */
 ...
 */
Player *player_create(Id id, char *name, Id location, Inventory *inv, int health)
{
    Player *p = NULL;
    ...
    if (player_set_inventory(p, inv) == ERROR)
    {
        free(p);
        return NULL;
    }
...
{
    if (player == NULL || object == NO_ID || inventory_is_full(player->inv) == TRUE)
    {
        return ERROR;
    }
    
    if (inventory_add_object(player->inv, object) == ERROR)
    {
        return ERROR;
    }
...
STATUS player_del_object_inventory(Player *player, Id object)
{
    ...
    if (inventory_remove_object(player->inv, object) == ERROR)
    {
        return ERROR;
    }
...
Set *player_get_objects_inventory(Player *player)
{
    ...
    return (inventory_get_objects(player->inv));
}

/** This function check if the id(input) is in inventory
 */
