 ...
 *
 * @file space.c
 * @author Profesores PPROG, Óskar, Javi, Joaquin, Luis
 * @version 3.0
 * @date 12-03-2022
...
struct _Space
{
    ...
    char description_l[SIZE_LONG_DESC];     /*!< Long description of the space */
    char description_s[SIZE_SHORT_DESC];    /*!< Short description of the space */
    BOOL light;                             /*!< Light value of the the space*/
};
...
Space *space_create(Id id)
{
    ...
    strcpy(newSpace->description_l, "WARNING: UNKNOWN SPACE");
    strcpy(newSpace->description_s, "UNKNOWN");
...
*/
STATUS space_set_description_large(Space *space, char *desc) {
    if (space == NULL || desc == NULL) {
        return ERROR;
    }

    strcpy(space->description_l, desc);
...
char* space_get_description_large (Space *space) {
    if (space == NULL) {
        return NULL;
    }

    return space->description_l;
}

STATUS space_set_description_short(Space *space, char *desc) {
    if (space == NULL || desc == NULL) {
        return ERROR;
    }

    strcpy(space->description_s, desc);
...
/*it gets the description of the space*/
char* space_get_description_short (Space *space) {
    if (space == NULL) {
        return NULL;
    }

    return space->description_s;
}
...
/*it gets the value of the light
*/
