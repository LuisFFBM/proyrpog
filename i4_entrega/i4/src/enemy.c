...
Enemy *enemy_create(Id id, char *name, Id location, int health)
{
    ...
    enem = (Enemy *)malloc(sizeof(Enemy));
    ...
    if (enemy_set_id(enem, id) == ERROR)
    {
        free(enem);
        return NULL;
    }

    if (enemy_set_location(enem, location) == ERROR)
    {
        free(enem);
        return NULL;
    }
    ...
    enem->name = NULL;
    if (enemy_set_name(enem, name) == ERROR)
    {
        free(enem);
        return NULL;
    }
    if (enemy_set_health(enem, health) == ERROR)
    {
        free(enem);
        return NULL;
    }
